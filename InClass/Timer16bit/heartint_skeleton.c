...
int main() {
  ...
  DDRB    =                             //set port B bit five to output

//setup timer counter 1 as the pwm source

  TCCR1A |= (1<<WGM11) | (1<<WGM10) | (1<<COM1A1) | (1<<COM1A0);                           //fast pwm, set on match, clear@bottom, 
...
// (16,000,000)/(8 * 2^16) = 30 cycles/sec
  ...
  TCCR3A = (0<<WGM11) | (0<<WGM10) | (0<<COM1A1) | (0<<COM1A0);   //normal mode
